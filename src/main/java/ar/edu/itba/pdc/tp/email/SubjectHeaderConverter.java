...
class SubjectHeaderConverter {
	...
	private String decodeTransformAndEncode() {
		if (encoding.equalsIgnoreCase("B")) {
			...
			encodedText = Base64.encodeBase64(transformed.getBytes())
					.toString();
			return asEncodedLine(charset, encoding, encodedText);
		} else if (encoding.equalsIgnoreCase("Q")) {
			// Decode with Q-encoding
			QCodec qCodec = new QCodec();
			try {
				String text = qCodec.decode(asEncodedLine(charset, encoding,
						encodedText));
				String transformed = leetify(text);
				// System.out.println("Transformed:");
				String newHeaderValue = qCodec.encode(transformed);
				return newHeaderValue;
			} catch (DecoderException e) { // do nothing
			} catch (EncoderException e) { // do nothing
			}
		}
	...
	private String asEncodedLine(String charset, String encoding,
			String encodedText) {
